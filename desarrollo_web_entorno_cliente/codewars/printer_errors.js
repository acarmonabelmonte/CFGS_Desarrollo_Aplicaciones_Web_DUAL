...
function printerError(s) {
	if (s !== null || s !== undefined) {
		let errores  = s.match(/[nopqrstuwxyzNOPQRSTUVWXYZ]/gi);
		if (errores !== null) {
			return errores.length.toString() + "/" + s.length.toString();
		}
			return "0" + "/" + s.length.toString();
	}
}

console.assert(printerError("aaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbmmmmmmmmmmmmmmmmmmmxyz") === "3/56")
