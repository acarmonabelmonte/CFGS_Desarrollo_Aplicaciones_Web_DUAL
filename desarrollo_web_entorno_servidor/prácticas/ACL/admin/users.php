	...
	?>
        ...
        <form action="users.php" method="post">
            ...
            foreach ($aPerms as $k => $v)
            {
				...
				echo "<option value=\"1\"";
				if ($userACL->hasPermission($v['Key']) && isset($rPerms[$v['Key']]['inheritted']) && $rPerms[$v['Key']]['inheritted'] != true) { echo " selected=\"selected\""; }
				echo ">Allow</option>";
				echo "<option value=\"0\"";
				...
				if (isset($rPerms[$v['Key']]['inheritted']) && $rPerms[$v['Key']]['inheritted'] == true || !array_key_exists($v['Key'],$rPerms))
				{
					...
					if (isset($rPerms[$v['Key']]['inheritted']) && $rPerms[$v['Key']]['value'] === true )
					{
						$iVal = '(Allow)';
					} else {
						$iVal = '(Deny)';
